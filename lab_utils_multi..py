...
# Embedded file name: /home/jovyan/work/lab_utils_multi.py
# Compiled at: 2021-10-04 17:11:20
# Size of source mod 2**32: 21312 bytes
...
def plt_contour_wgrad(x, y, hist, ax, w_range=[
 -100, 500, 5], b_range=[-500, 500, 5], contours=[
...
def plt_contour_multi(x, y, w, b, ax, prange, p1, p2, title='', xlabel='', ylabel=''):
    contours = [
     ...
     1250.0, 1500.0, 10000.0, 100000.0, 1000000.0, 10000000.0]
    ...
    CS = ax.contour(px, py, z, contours, linewidths=2, colors=[
     'dlblue', 'dlorange', 'dldarkred', 'dlmagenta', 'dlpurple'])
...
def plt_equal_scale(X_train, X_norm, y_train):
    ...
    prange = [
     [
      0.193, 0.283, 50],
     [
      -25.818263190000003, -25.72826319, 50],
     [
      -50000, 0, 50],
     [
      -1500, 0, 50],
    ...
    b_best = 235
    plt_contour_multi(X_train, y_train, w_best, b_best, (ax[0]), prange, 0, 1, title='Unnormalized, J(w,b), vs w[0],w[1]',
    ...
    prange = [[61, 161, 75],
     ...
     [
      226, 526, 50]]
    plt_contour_multi(X_norm, y_train, w_best, b_best, (ax[1]), prange, 0, 1, title='Normalized, J(w,b), vs w[0],w[1]',
...
def plt_gradients(x_train, y_train, f_compute_cost, f_compute_gradient):
    ...
    qk = ax[1].quiverkey(Q, 0.9, 0.9, 2, '$2 \\frac{m}{s}$', labelpos='E', coordinates='figure')
    ax[1].set_xlabel('w')
    ax[1].set_ylabel('b')
...
def run_gradient_descent_feng(X, y, iterations=1000, alpha=1e-06):
    m, n = X.shape
    initial_w = np.zeros(n)
    initial_b = 0
